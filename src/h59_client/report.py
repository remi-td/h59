...
def _latest_blood_oxygen_sample(conn: sqlite3.Connection, device_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
...
def _latest_pressure_sample(conn: sqlite3.Connection, device_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
...
def _latest_hrv_sample(conn: sqlite3.Connection, device_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
        SELECT value, timestamp
        FROM hrv_samples
...
def render_health_dashboard_report(
    ...
    *,
    ...
    try:
        ...
        lines.append("")
        lines.append("### Blood Oxygen / SpO2")
        if historical_metrics["blood_oxygen_total"] > 0 and latest_blood_oxygen is not None:
            lines.append(
                f"- Historical samples: `{historical_metrics['blood_oxygen_total']}` total, "
                f"`{historical_metrics['blood_oxygen_daily']}` on selected day"
            )
            lines.append(
                f"- Latest historical sample: `min={latest_blood_oxygen['min_percent']}%`, "
                f"`max={latest_blood_oxygen['max_percent']}%` at `{_fmt_ts(latest_blood_oxygen['timestamp'])}`"
            )
        elif "spo2" in realtime:
            entry = realtime["spo2"]
        ...
        lines.append("")
        lines.append("### HRV")
        if historical_metrics["hrv_total"] > 0 and latest_hrv is not None:
            lines.append(
                f"- Historical samples: `{historical_metrics['hrv_total']}` total, "
                f"`{historical_metrics['hrv_daily']}` on selected day"
            )
            lines.append(f"- Latest historical sample: `{latest_hrv['value']}` at `{_fmt_ts(latest_hrv['timestamp'])}`")
        elif "hrv" in realtime:
            entry = realtime["hrv"]
            lines.append(f"- Realtime sample only: `{entry['value']}` at `{_fmt_ts(entry['timestamp'])}`")
        else:
            lines.append("- Not available in the current database.")
        lines.append("")
        lines.append("### Stress / Pressure-like")
        if historical_metrics["pressure_total"] > 0 and latest_pressure is not None:
            lines.append(
                f"- Historical samples: `{historical_metrics['pressure_total']}` total, "
                f"`{historical_metrics['pressure_daily']}` on selected day"
            )
            lines.append(f"- Latest historical sample: `{latest_pressure['value']}` at `{_fmt_ts(latest_pressure['timestamp'])}`")
        elif "pressure" in realtime:
            entry = realtime["pressure"]
            lines.append(f"- Realtime sample only: `{entry['value']}` at `{_fmt_ts(entry['timestamp'])}`")
        elif "fatigue" in realtime:
            entry = realtime["fatigue"]
            lines.append(f"- Realtime fatigue-like sample only: `{entry['value']}` at `{_fmt_ts(entry['timestamp'])}`")
        else:
            lines.append("- Not available in the current database.")
        lines.append("")
