...
import type { MetricPoint, MetricSeriesResponse, SleepResponse, SleepSessionSummary } from "../api/types";
...
function rollingSevenDays(endDate: string | null): DaySlot[] {
...
function stepsOption(points: MetricPoint[], slots: DaySlot[]): EChartsOption | null {
    ...
    series: [
      {
        ...
        itemStyle: { color: "#c27a31", borderColor: "#fffaf2", borderWidth: 2 },
        areaStyle: {
          color: {
            type: "linear",
            x: 0,
...
function heartOption(points: MetricPoint[], slots: DaySlot[]): EChartsOption | null {
...
function sleepOption(sessions: SleepSessionSummary[], slots: DaySlot[]): EChartsOption | null {
...
function latestStepsSummary(points: MetricPoint[]): string | null {
...
function latestHeartSummary(points: MetricPoint[]): string | null {
...
function latestSleepSummary(sessions: SleepSessionSummary[]): string | null {
...
export function Trends({ device }: { device: string }) {
  ...
  useEffect(() => {
    ...
    ]).then(([stepsPayload, heartPayload, sleepPayload]) => {
      if (!cancelled) {
  ...
  const slots = useMemo(() => rollingSevenDays(endDate), [endDate]);

  const stepsChart = useMemo(() => stepsOption(steps?.points || [], slots), [steps, slots]);
  const heartChart = useMemo(() => heartOption(heart?.points || [], slots), [heart, slots]);
  const sleepChart = useMemo(() => sleepOption(sleep?.sessions || [], slots), [sleep, slots]);
